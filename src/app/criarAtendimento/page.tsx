    ...
    <div className="flex justify-center mt-[3rem] font-[family-name:var(--font-gabarito)]">
      <div className="flex flex-col items-center w-[80vw] lg:w-[60vw] gap-6">
        ...
        <div className="flex flex-col bg-[#1f5c77] p-6 gap-2 w-full shadow-2xl">
          <div className="flex">
            <div className="w-full">
              ...
              <SearchBarInteractive value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />
            </div>
            <button 
            type="button"
          ...
          {noSearchResult && <div className="text-red-500 text-center font-normal">Sem resultado da pesquisa</div>}

          {errors.patient && <div className="text-red-500 text-center font-normal">Selecione um paciente</div>}
          ...
          {filteredPatients.map((p, i) => (
            <label className="flex" key={i}>
              <input type="radio" value="49271947878" className="peer hidden" {
                ...register("patient", {required: true})
              }/>
              <div className={`flex w-full peer-checked:bg-blue-200 bg-white p-1.5 rounded transition-colors duration-150`}>

                <div className={`flex-7 truncate whitespace-nowrap overflow-hidden`}>
                  {p}
                </div>
        ...
        <div className="flex justify-evenly w-full">
            <button className="bg-[rgb(56,163,165)] p-2 text-white text-2xl font-bold rounded min-w-[9rem] shadow-2xl hover:opacity-70 transition-opacity duration-150 ease-in-out cursor-pointer">Editar</button>
            <button 
